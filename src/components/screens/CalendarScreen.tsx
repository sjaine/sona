...
function EventDetail({
  ...
  event: CalEvent;
  onClose: () => void;
  onRsvpChange: (id: string, rsvp: CalEvent["rsvp"]) => void;
}) {
          ...
          style={{
            background:
              event.scope === "Team"
                ? "var(--accent-light)"
                : "var(--green-light)",
            color: event.scope === "Team" ? "var(--accent)" : "var(--green)",
          }}
        >
          {event.scope}
        </div>
          ...
          <div className="flex -space-x-1.5">
            {[0, 1, 2, 3].map((i) => (
              <div
                ...
                style={{
                  background: ["#C8E8F8", "#DCF5E8", "#FEF3DC", "#EDE9FF"][i],
                }}
              />
            ))}
          </div>
        ...
        <div className="mb-4">
          ...
          {event.locationAddress ? (
            <>
              ...
              <button
                className="text-xs font-semibold mt-1 px-3 py-1 rounded-full pressable"
                style={{
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pb-4">
        {/* Image upload */}
        <div
          className="w-full h-36 rounded-2xl mb-4 flex items-center justify-center"
          style={{
            background: "var(--surface-2)",
          ...
          <div className="flex flex-wrap gap-1.5">
            {tags.map((t) => (
              <button
                ...
                className="chip chip-accent text-xs pressable"
              >
                {t} ×
              </button>
            ))}
        ...
        <div className="mb-3">
          ...
          {(["Team", "Organization"] as const).map((v) => (
            <div
              key={v}
              className="flex items-center justify-between py-2.5 pressable"
            ...
            >
              <span
                ...
                style={{ color: "var(--text-primary)" }}
              >
                {v === "Organization" ? "Organization (anyone)" : v}
              </span>
              <div
                className="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                style={{
                  borderColor:
                    visibility === v ? "var(--accent)" : "var(--border)",
                }}
              >
                {visibility === v && (
                  <div
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="Write a little about the event..."
            value={description}
    ...
    <>
      ...
      <div className="grid grid-cols-7 px-3 mb-1 flex-shrink-0">
        {DAY_NAMES.map((d, i) => (
          <div
            key={i}
            className="text-center py-1.5 text-xs font-semibold"
                ...
                <div className="flex gap-0.5 mt-0.5">
                  {(eventsByDay[day!] || []).slice(0, 2).map((ev, j) => (
                    <div
                      ...
                      style={{
                        background:
                          ev.scope === "Team" ? TEAM_COLOR : ORG_COLOR,
                      }}
                    />
                  ))}
                </div>
      ...
      <div className="overflow-y-auto scrollbar-hide flex-1 pb-2">
        {selectedDay && (
          <div
            className="px-4 pb-1 text-sm font-bold"
            style={{ color: "var(--text-secondary)" }}
          >
        ...
        {selectedEvents.map((ev) => (
          <button
            ...
            className="w-full px-4 mb-2 text-left pressable"
          >
            <div
...
export default function CalendarScreen() {
  ...
  function updateRsvp(id: string, rsvp: CalEvent["rsvp"]) {
    setEvents((prev) => prev.map((e) => (e.id === id ? { ...e, rsvp } : e)));
    setSelectedEvent((prev) => (prev?.id === id ? { ...prev, rsvp } : prev));
  }
